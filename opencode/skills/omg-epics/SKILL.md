...
3. **Write it to the docs tree** at the resolver-computed path (`<docs_base>/build-report/<id>.md`). Give it a `hindsight` block — every report ships, even one with nothing to record: at minimum it records that the spec was built and when the build completed. Only the user removes the block.
