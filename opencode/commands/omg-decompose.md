...
Drive the plan phase over the epic identified above, using the `omg-decompose` skill end to end. Pass the mode reported above — fresh mint or refinement pass — through to each planner, and reconcile the terminal beads from the graph's actual state. When the graph is built and validated, show me the final structure and stop for my review.
