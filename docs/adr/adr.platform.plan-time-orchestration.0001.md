...
The build-time coordinator — the **foreman** (`omg-foreman`) — is deliberately minimal: it grabs a ready bead, reads its `agent` label, dispatches, and holds no orchestration state (`adr.platform.beads-sync-ownership.0001` names this the orchestration layer; the foreman skill and agent state it as the routing invariant). That statelessness is the property that lets a new kind of worker appear in an epic and be routed with zero foreman change. It is load-bearing and the PRD (`prd.platform.test-planning.0002`, R9/Goal 6) declares it sacred.

**What makes that statelessness *sound* is an assumption on the workers, which PRD R17 (added after this ADR was first drafted) turns into a stated contract.** The foreman holds no state, so its *only* input is the current bead state; label-only routing over bead state is correct only if every dispatched worker leaves its bead in a state the foreman can act on next pass. R17 names this the **dispatch-lifecycle contract**: a foreman-dispatched build-phase worker must return its bead **closed** (success) or **reopened-and-blocked-by-a-new-bead** (couldn't finish) — never `in_progress` (which `bd ready` hides, silently wedging the epic) and never reopened-but-unblocked (which re-dispatches to the same failed agent in a loop). Statelessness is not an independent luxury; it is *earned* by pushing this guarantee into the workers. A foreman that instead inspected returned beads to reason about "abandoned vs. in-flight" would rebuild the very orchestration state this decision forbids. R17 is therefore the soundness basis of the routing invariant, not a separate concern — and it is why the invariant can be declared sacred without qualification.

**The one case the worker contract cannot cover is a worker that never returns at all — a crash, kill, or power loss (PRD R18).** Such a worker leaves its bead `in_progress` with no live worker; `bd ready` hides it and the stateless foreman never re-dispatches it, so the epic silently wedges. Recovery must not be bought by giving the foreman run-state ("did I dispatch this bead this run?"), because that is the orchestration state the invariant forbids. The recovery discipline is therefore designed to stay stateless (see Decision §6 and Consequences), which is why R18 belongs to this decision's neighborhood: it is a constraint on *how* recovery may be built so the routing invariant survives it.
...
6. **Crash recovery must preserve the foreman's statelessness (PRD R18).** A worker that never returns (crash/kill/power loss) leaves its bead `in_progress`, invisible to `bd ready`, and a stateless foreman would never re-dispatch it — so recovery is necessary. But recovery must be built so it adds **no orchestration state and no routing intelligence**: it may extend the foreman's *looping mechanics* (as build-mode mechanics already may), never its routing. In particular, the primary detection is a **run-start orphan scan** — before dispatching anything on a fresh run, any `in_progress` child is orphaned *by definition*, so the foreman needs no "did I dispatch this?" bookkeeping — chosen over close-time or dispatch-time detection precisely because those would force the foreman to remember what it dispatched, rebuilding the forbidden state. Recovery is also **bounded** (one automatic retry, then a human gate) so it cannot spin. The concrete detection points, recovery path, and boundedness proof are the design doc's territory; this ADR fixes only the constraint that recovery lives in looping mechanics and preserves statelessness, so the routing invariant survives it.

This decision **records the phase model and the separation of the two orchestration roles.** *How* the orchestrator drives each pass — subagent dispatch versus plan-time dependency beads versus another shape — is specified in `design.platform.test-planning.0002` and is beneath this boundary; it may change without revisiting this decision. Likewise the dispatch-lifecycle contract (§2) and the recovery mechanics (§6) are *stated as constraints* here and *realized* in the design doc.
...
- **The routing invariant is now *justified*, not merely asserted.** The dispatch-lifecycle contract (§2) names *why* the foreman can be stateless — workers guarantee clean bead state on return — so "the foreman holds no state" reads as a reasoned property with an explicit precondition, not an article of faith. A reviewer can now flag a worker that leaves a bead `in_progress` as a defect against a named contract.
- **A crashed epic is recoverable without breaking statelessness.** Because recovery is constrained to looping mechanics with run-boundary detection (§6), a crash-interrupted epic resumes on re-run and the foreman still holds no orchestration state. The workflow gains crash-resilience *and* keeps the invariant that makes it simple.
...
- **The dispatch-lifecycle contract is a discipline every build-phase worker must honor, not a harness-enforced guarantee.** Like the plan-time sequencing obligation, it is trust-the-runbook: a worker whose runbook omits it can violate it, and the violation surfaces only as a stranded `in_progress` bead caught by recovery's drain-time check, not at authorship. Accepted as consistent with the workflow's existing trust model.
- **Crash recovery carries a bounded, accepted residual risk.** Run-boundary detection (§6) recovers only at run-start/drain-time, not mid-run, and re-dispatch relies on the replacement agent rediscovering partial work — a chained failure can duplicate or conflict. This is accepted as the price of keeping the foreman stateless; a future session-id-claimant scheme (named-deferred in the design doc and PRD) would narrow it without changing this decision.

## Related Documents

- `prd.platform.test-planning.0002` — the PRD this ADR was produced for; its R1 (split the decomposer into three plan-time roles), R9 (foreman routing sacred, looping mechanics touchable), and R12 (verification is standard) are the requirements this decision realizes at the architectural level. **R17 (the dispatch-lifecycle contract) and R18 (crash recovery), with PRD Goal 9, refined this ADR in place** — R17 as the soundness basis of the foreman's statelessness (Context, Decision §2) and R18 as the constraint that recovery preserve statelessness (Decision §6). These reinforce the routing invariant rather than amending it; no new ADR was minted, because they record no new standing decision — only *why* the existing statelessness decision holds and *how* recovery must respect it.
